...
const Navigation = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 glass-strong border-t border-white/10 z-50 shadow-elevation">
      <div className="flex justify-around items-center h-24 max-w-2xl mx-auto px-6">
        {navItems.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
            className={({ isActive }) =>
              `flex flex-col items-center justify-center gap-2 px-6 py-3 rounded-2xl transition-all duration-300 ${
                isActive
                  ? "text-primary scale-110 bg-primary/10 shadow-glow border border-primary/30"
          ...
          >
            {({ isActive }) => (
              <>
                <item.icon
                  className={`w-7 h-7 transition-all ${isActive ? "drop-shadow-glow scale-110" : ""}`}
                />
